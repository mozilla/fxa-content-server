...
const SETTINGS_URL = `${config.fxaContentRoot}settings?showSecurityPanel=true`;
...
registerSuite('TOTP', {
  ...
  tests: {
    'does not show panel when query `showSecurityPanel` is not set': function () {
      return this.remote
    ...
    'can add TOTP to account and confirm web login': function () {
      return this.remote
        ...
        .then(type(selectors.TOTP_SIGNIN.INPUT, 'INVALID'))
    ...
    'can add TOTP to account and confirm sync login': function () {
      return this.remote
        // Shows success for confirming token
        .then(type(selectors.TOTP.CONFIRM_CODE_INPUT, generateCode(secret)))
        .then(click(selectors.TOTP.CONFIRM_CODE_BUTTON))
    ...
    'can add TOTP to account and confirm oauth login': function () {
      return this.remote
        // Shows success for confirming token
        .then(type(selectors.TOTP.CONFIRM_CODE_INPUT, generateCode(secret)))
        .then(click(selectors.TOTP.CONFIRM_CODE_BUTTON))
