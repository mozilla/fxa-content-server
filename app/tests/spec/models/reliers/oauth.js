  ...
  describe('models/reliers/oauth', function () {
    ...
    describe('fetch', function () {
      describe('signin/signup flow', () => {
        it('populates expected fields from the search parameters', function () {
          ...
          return relier.fetch()
            .then(function () {
              ...
              // PKCE parameters
              assert.equal(relier.get('code_challenge'), CODE_CHALLENGE);
              assert.equal(relier.get('code_challenge_method'), CODE_CHALLENGE_METHOD);
            });
        });
        ...
        it('throws if invalid PKCE params are specified', () => {
          windowMock.location.search = TestHelpers.toSearchString({
            access_type: ACCESS_TYPE,
            action: ACTION,
            client_id: CLIENT_ID,
