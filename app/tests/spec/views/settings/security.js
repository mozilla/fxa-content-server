...
define(function (require, exports, module) {
  ...
  const View = require('views/settings/security');

  describe('views/settings/security', () => {
    let account;
    let email;
    let metrics;
    let notifier;
    ...
    function initView() {
      ...
      sinon.stub(view, '_showSecurityPanel').callsFake(() => featureEnabled);
    ...
    beforeEach(() => {
      ...
      sinon.stub(account, 'createTotpToken').callsFake(() => {
        return Promise.resolve({
          qrCodeUrl: 'https://immaimage.com/pic.png',
          secret: 'MZEE 4ODK'
        });
      });
    ...
    describe('feature disabled', () => {
      ...
      it('should remove panel if `showSecurityPanel` query is not specified', () => {
    ...
    describe('feature enabled', () => {
      ...
      describe('should show token status', () => {
        ...
        it('should show token status view if user has token', () => {
          assert.equal(view.$('#security-status .enabled').length, 1);
      ...
      describe('should create new token', () => {
        ...
        it('should show QR code', () => {
          ...
          assert.equal(view.$('img.qr-image').attr('src'), 'https://immaimage.com/pic.png');
        });

        it('should not show status section', () => {
          assert.equal(view.$('.security-list.hidden').length, 1);
        });
